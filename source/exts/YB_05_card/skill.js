...
const skill = {
	...
	ybsl_anduchencang_skill: {
		...
		content: function () {
			...
			next.set('filterCard', function (card, player) {
				if (get.name(card) != 'ybsl_anduchencang') {
					return false;
				}
				return true;
			});
		...
		ai: {
			effect: {
				target: function (card, player, target) {
					...
					if (card.name == 'sha' && get.suit(card) == 'heart') {
						return 'zerotarget';
					}
				},
			},
	...
	rewrite_qinglong_skill: {
		...
		content: function () {
			...
			player
				...
				.set('addCount', false).logSkill = 'rewrite_qinglong_skill';
		},
	},
	...
	rewrite_yitianjian: {
		...
		content: function () {
			'step 0';
			player.chooseToDiscard('h', get.prompt('rewrite_yitianjian'), '弃置一张手牌并回复1点体力').set('ai', (card) => 7 - get.value(card)).logSkill = 'rewrite_yitianjian';
			('step 1');
			if (result.bool) {
				player.recover();
			}
	...
	rewrite_fulei_skill: {
		...
		content: function () {
			...
			player
				...
				.set('prompt2', '重铸后，若此牌不为红手牌数小于体力值，则摸一张牌；<br>若此牌为红且体力值小于手牌数，则恢复一点体力')
				.set('ai', function (card) {
