...
const dynamicTranslate = {
	...
	ybsl_xianyin(player) {
		//弦音
		const str0 = `转换技,(括号内的阴阳为${get.poptip('ybsl_luanming')}的形态)<br/>`;
		const str1 = '阳(阳):当你因弃置而失去一张♠️️️牌时,你可令一名角色下个摸牌阶段额外摸一张牌;';
		const str2 = '阳(阴):当你因弃置而失去一张♣️️️牌时,你可令一名角色回复1点体力;';
		const str3 = '阳(阳):当你因弃置而失去一张♥️️️牌时,你可令一名角色失去1点体力;';
		const str4 = '阴(阴):当你因弃置而失去一张♦️️️牌时,你可令一名角色下个摸牌阶段少摸一张牌';
	...
	ybsl_kanxiao(player) {
		const num = player.getStorage('ybsl_kanxiao') - 0 + 1;
		return '蓄力技(1/3).每回合限' + get.cnNumber(num) + '次,①当你失去手牌后,你可以消耗1点蓄力值回复一点体力;②当你受到伤害后或失去体力后,你可以消耗1点蓄力值摸两张牌';
	},
	ybsl_shipin(player) {
		const num = player.getStorage('ybsl_shipin') - 0 + 1;
	...
	zxunnamed_huaiyi(player, skill) {
		...
		const list = [...get.skillInfoTranslation('zxunnamed_shixian', player).matchAll(/(?<=\d\.).*?(?=,\d\.|;)/g)];
