...
const skill = {
	YB_wannaw_bingdong: {
		...
		cost() {
			event.result = player
				...
				.set('prompt', get.prompt2('YB_wannaw_bingdong'))
				.forResult();
		...
		content: async function (event, trigger, player) {
			...
			if (cards.length && cards.length > 1) {
				...
				const types = quchong(cards.map((card) => get.type2(card, target))).length == 1;
				const suits = quchong(cards.map((card) => card.suit)).length == 1;
				const namenums = quchong(cards.map((card) => get.cardNameLength(card, target))).length == 1;
				const numbers = quchong(cards.map((card) => card.number)).length == 1;
				if (types) {
		...
		subSkill: {
			kill: {
				...
				content() {
					...
					if (!player.storage.counttrigger.North_huimiex) {
						player.storage.counttrigger.North_huimiex = 0;
					}
	...
	North_icedamage: {
		...
		ai: {
			...
			effect: {
				target(card, player, target) {
					if (card.name == 'sha' && get.nature(card) == 'ice') {
						return 'zerotarget';
					}
				},
			},
	...
	North_panding: {
		...
		subSkill: {
			a: {
				...
				forced: true,
				filter(event, player) {
					// game.log('_status.event.parent:',_status.event.parent)
					...
					// if(event.parent.name)game.log('event.parent.name:',event.parent.name)
					// if(event.parent.skill)game.log('event.parent.skill:',event.parent.skill)
					...
					// 	if(event.parent.name=='ygjbt')return true;
					// }
					// return false;
					return true;
				},
	...
	North_yhy_xuyin6: {
		...
		filter(event, player) {
			if (player.countMark('North_yhy_xuyin4') > 0) {
				return true;
			}
			if (player.countMark('North_yhy_xuyin5') > 0) {
				return true;
			}
			return false;
		},
	...
	North_yhy_cihua: {
		...
		content() {
			...
			if (result.targets[0]) {
				...
				if (result.targets[0].hasMark('North_yhy_xuyin5')) {
					list.push('狭');
				}
				...
				player
					.chooseTarget(2, '是否转移<悯>标记？', function (card, player, target) {
						if (ui.selected.targets.length) {
							return !target.hasMark('North_yhy_minzeng_min');
						}
						return target.countMark('North_yhy_minzeng_min') > 0;
					})
			...
			if (
				...
				player
					.chooseTarget(2, '是否转移<憎>标记？', function (card, player, target) {
						if (ui.selected.targets.length) {
							return !target.hasMark('North_yhy_minzeng_zeng');
						}
						return target.countMark('North_yhy_minzeng_zeng') > 0;
					})
					.set('complexTarget', true)
	...
	North_yhy_minzeng_minyi: {
		...
		filter(event, player) {
			if (event.player != player) {
				return event.player.hasMark('North_yhy_minzeng_min');
			}
		},
		nobracket: true,
		charlotte: true,
	...
	North_yhy_minzeng_zengyi: {
		...
		filter(event, player) {
			if (event.player != player) {
				return event.player.hasMark('North_yhy_minzeng_zeng');
			}
		},
		nobracket: true,
		charlotte: true,
		...
		content() {
			...
			if (list.length) {
				player
					...
					.set('prompt', get.prompt('North_yhy_minzeng_zengyi'))
					.set('prompt2', get.translation('North_yhy_minzeng_zengyi_info'))
					.set('ai', function () {
	...
	North_dy_qingyu: {
		...
		intro: {
			...
			content(storage, player, skill) {
				let str = '';
				const list = player.storage.North_dy_qingyu_wuku;
				let list2 = player.storage.North_dy_qingyu_light;
				let list3;
				if (player.storage.North_dy_zhengwu_length) {
					list3 = player.storage.North_dy_zhengwu_length;
				} else {
					list3 = [];
				}
				str += '';
		...
		up(player) {
			const next = game.createEvent('North_dy_qingyu_light', false);
			next.player = player;
			next.setContent(lib.skill.North_dy_qingyu.upc);
		},
		upc() {
			'step 0';
			const dialog = [get.prompt('North_dy_qingyu_light')];
			const list1 = player.storage.North_dy_qingyu_light;
			const list2 = player.storage.North_dy_qingyu_wuku.filter(function (i) {
			...
			player
				...
				.set('filterButton', function (button) {
					const player = _status.event.player;
					if (player.storage.North_dy_qingyu_light.includes(button.link[2])) {
						return false;
					}
					return true;
				});
			...
			if (player.storage.North_dy_qingyu_light.length >= 6 && !player.hasSkill('North_dy_poshi')) {
				...
				game.log(player, '获得了技能', '#yNorth_dy_poshi');
		...
		subSkill: {
			light: {
				...
				enable: 'phaseUse',
				filter(event, player, card) {
					if (player.countMark('North_dy_qingyu_mark') >= player.hp) {
						return false;
					}
					return (
				...
				content() {
					...
					player.draw();
					('step 1');
					if (!player.hasSkill('North_dy_qingyu_mark')) {
						player.addTempSkill('North_dy_qingyu_mark');
					}
					player.addMark('North_dy_qingyu_mark', 1, false);
					('step 2');
					let list1 = player.storage.North_dy_qingyu_light;
					let list2 = player.storage.North_dy_qingyu_wuku.filter(function (i) {
						return !list1.includes(i);
					});
					if (list2.length) {
						lib.skill.North_dy_qingyu.up(player);
					}
				},
				ai: {
					basic: {
			...
			use1: {
				...
				filter(event, player) {
					...
					const list = player.storage.North_dy_qingyu_light;
			...
			use2: {
				...
				filter(event, player) {
					...
					if (event.name.indexOf('lose') != 0) {
						return list.includes(event.card.name) && get.type(event.card) == 'basic' && event.card.isCard;
					}
				},
	...
	North_dy_zhengwu: {
		...
		content() {
			...
			event.count--;
			player
				.chooseTarget([1, 3], get.prompt2('每当你回合开始前或回合外受到一点伤害后,你可令至多3名角色依次摸三张牌并弃一张牌,再令至多3名角色进入横置状态'), function (card, player, target) {
		...
		subSkill: {
			use1: {
				...
				filter(event, player) {
					...
					let max = 1;
					if (player.storage.North_dy_zhengwu_plus == true) {
						max = 3;
					}
					if (player.countMark('North_dy_zhengwu_block') >= max) {
						return false;
					}
					...
					for (const i of player.storage.North_dy_qingyu_light) {
				...
				chooseButton: {
					dialog(event, player) {
						...
						for (const i of player.storage.North_dy_qingyu_light) {
							...
							if (!player.storage.North_dy_zhengwu_length) {
								...
								if (!player.storage.North_dy_zhengwu_length.includes(i)) {
									list.push(['整武', '', i]);
								}
								if (i == 'sha') {
									for (let j of lib.inpile_nature) {
					...
					backup(links, player) {
						return {
							...
							precontent() {
								if (!player.hasSkill('North_dy_zhengwu_length')) {
									player.addTempSkill('North_dy_zhengwu_length');
								}
								if (!player.storage.North_dy_zhengwu_length) {
									player.storage.North_dy_zhengwu_length = [];
								}
								player.storage.North_dy_zhengwu_length.push(event.result.card.name);
								if (!player.hasSkill('North_dy_zhengwu_block')) {
									player.addTempSkill('North_dy_zhengwu_block');
								}
								player.addMark('North_dy_zhengwu', 1, false);
	...
	North_ssx_jibing: {
		...
		content() {
			...
			('step 2');
			const list = [];
			for (let j = event.cards.length; j > 0; j--) {
	...
	North_ssx_lieyuan: {
		...
		content() {
				...
				player
					.chooseTarget(function (card, player, target) {
						if (get.position(event.card) == 'e') {
							...
							const type = get.subtype(event.card);
			...
			player
				...
				.set('ai', function () {
					let att = get.attitude(_status.event.player, event.target);
					if (att < 0) {
						return '是';
					}
					return '否';
		...
		content() {
			...
			player.chooseCardTarget({
				...
				filterTarget(card, player, target) {
					...
					if (get.type(cardx) == 'equip' && !target.canEquip(cardx, true)) {
						return false;
					}
	...
	North_zgl_dongxu: {
		...
		subSkill: {
			...
			link: {
				...
				content() {
					'step 0';
					let num = player.getHistory('lose', function (evt) {
						const ll = [];
						ll.add(evt.hs);
						ll.add(evt.es);
						for (const i of ll) {
	...
	North_zgl_shiyan: {
		...
		filter(event, player) {
			...
			const cards = player.getExpansions('North_zgl_qizhu');
		...
		subSkill: {
			dis: {
				...
				content() {
					...
					event.num = suit.length;
					('step 1');
					let str = '是否选择至多' + get.cnNumber(event.num) + '名角色,对其依次造成' + get.cnNumber(event.num) + '点火焰伤害？';
	...
	North_zgl_shiyanxx: {
		...
		filter(event, player) {
			...
			if (player.countMark('North_zgl_shiyanxx_mark') >= suit.length) {
				return false;
			}
			for (const i of cards) {
				if (i.suit == event.card.suit) {
		...
		subSkill: {
			dis: {
				...
				content() {
					...
					event.num = suit.length;
					('step 1');
					let str = '是否选择至多' + get.cnNumber(event.num) + '名角色,对其依次造成两点火焰伤害？';
					player.loseToDiscardpile(player.getExpansions('North_zgl_qizhu'));
					player.chooseTarget([0, event.num], str).set('ai', function (target) {
						return -get.attitude(_status.event.player, target);
					});
	...
	North_ld_chenxun: {
		...
		content() {
			...
			const skills = event.tar.getSkills(null, false, false).filter(function (i) {
				const info = get.info(i);
				return info && !info.charlotte;
			});
			const cards = get.cards(skills.length + 2);
			game.cardsGotoOrdering(cards);
			// event.tar=(player.storage.North_ld_chenxun||player);
			const next = player.chooseToMove('忱恂:获得其中两张牌', true);
		...
		subSkill: {
			after: {
				...
				forced: true,
				filter(event, player) {
					if (player.getExpansions('North_ld_chenxun').length) {
						return true;
					}
					return false;
				},
				content() {
					'step 0';
					const cards = player.getExpansions('North_ld_chenxun');
					...
					if (result.bool) {
						...
						if (!tar.hasSkill('North_ld_chenxun')) {
							...
							let numb = player.countCards('h');
							const numc = player.maxHp - numb;
							if (numc > 0) {
								player.draw(numc);
							}
						...
						const next = tar.phaseDraw();
						event.next.remove(next);
						trigger.next.push(next);
						const next1 = tar.phaseUse();
		...
		subSkill: {
			eq: {
				...
				content() {
					'step 0';
					player
						.chooseTarget(get.prompt2('North_smk_shangying_eq'), true, function (card, player, current) {
							return current.getEquip(1);
						})
						.setHiddenSkill(event.name).ai = function (target) {
							...
							const player = _status.event.player;
							const bl = get.attitude(player, target);
							return num * bl;
						};
					('step 1');
					if (result.bool) {
			...
			qz1: {
				...
				audio: 'gzjili',
				filter(event, player) {
					if (!lib.skill.olyajiao.filter(event, player)) {
						return false;
					}
			...
			qz2: {
				...
				inherit: 'oltiaoxin',
				filter(event, player) {
					if (!lib.skill.oltiaoxin.filter(event, player)) {
						return false;
					}
			...
			qz4: {
				...
				inherit: 'retieji',
				filter(event, player) {
					if (!lib.skill.retieji.filter(event, player)) {
						return false;
					}
	...
	North_dc_ruofu: {
		...
		content() {
			...
			if (
				...
				let str = '本次第' + get.cnNumber(event.count + 1) + '次发动<br>摸一张牌,并将一张牌置于其他角色上称为<若芙>';
				player.chooseBool(get.prompt('North_dc_ruofu', player), str).set('ai', function () {
					const targets = game.countPlayer(function (current) {
						if (current == _status.event.player) {
							return false;
						}
						if (current.getExpansions('North_dc_ruofu_mark').length) {
							return false;
						}
						let att = get.attitude(_status.event.player, current);
						return att <= 0;
					});
					return targets > 0;
				});
	...
	North_dc_sulian: {
		...
		filter(event, player) {
			if (!event.source || !event.source.getExpansions('North_dc_ruofu_mark').length) {
				return false;
			}
			if (!event.player.isAlive() || !event.player.getExpansions('North_dc_ruofu_mark').length) {
				return false;
			}
			return true;
		},
	...
	North_dc_ziman: {
		...
		content() {
			...
			if (
				...
				let str = '本次第' + get.cnNumber(event.count + 1) + '次发动<br>收回一名角色的<若芙>牌';
				if (event.count == 0) {
					str += '本次收回,你回复一点体力';
				}
				if (event.count == 1) {
					str += '本次收回,令本次选择的角色流失1点体力';
				}
				if (event.count >= 2) {
					str += '本次收回,你与本次选择的角色互换位置且其下个回合改为由你操控';
				}
			...
			if (result.bool) {
				...
				switch (event.count) {
					...
					case 2:
						...
						if (get.mode() != 'tafang' && get.mode() != 'chess') {
							player.storage.North_dc_ziman.push(target);
						}
						break;
				}
			}
	...
	North_bmh_huanchao: {
		...
		content() {
			...
			game.filterPlayer(function (current) {
				const skill = 'North_bmh_wuzhi_' + event.huanchao;
			...
			ben: {
				...
				intro: {
					content(storage, player, skill) {
						let str = '';
						if (player.storage.North_bmh_huanchao_ben == true) {
							str += '限定技.转换技.出牌阶段,你可弃置三张不同类型的牌,并执行,<span class=thundertext>阳:令拥有<献>的角色依次回复X点体力</span>;阴:令拥有<率>的角色依次流失X点体力.执行结束后,相关角色弃置<巫治>标记(X为此技能发动次数)';
						} else {
							str += '限定技.转换技.出牌阶段,你可弃置三张不同类型的牌,并执行,阳:令拥有<献>的角色依次回复X点体力;<span class=thundertext>阴:令拥有<率>的角色依次流失X点体力</span>.执行结束后,相关角色弃置<巫治>标记(X为此技能发动次数)';
						}
						str += '<br>当前已使用';
	...
	North_bmh_chizhang: {
		...
		filter(event, player) {
			if (
				game.countPlayer(function (target) {
					if (get.North_bmh_chizhang(target).length) {
						return true;
					}
				}) < 1
			) {
		...
		content() {
			...
			player
				.chooseTarget(function (card, player, target) {
					return get.North_bmh_chizhang(target).length;
				})
	...
	North_bmh_chizhangd: {
		...
		filter(event, player) {
			...
			const damage = player.getHistory('sourceDamage').length;
			let num = player.maxHp + player.getHandcardLimit();
			if (damage >= num) {
				return true;
			}
		},
	...
	North_bmh_chizhangt: {
		...
		filter(event, player) {
			...
			const damage = player.getHistory('useCard').length;
			let num = player.maxHp + player.getHandcardLimit();
			if (damage >= num) {
				return true;
			}
	...
	North_bmh_chizhangp: {
		...
		filter(event, player) {
			...
			const damage = player.getHistory('lose').length;
			let num = event.player.getHistory('lose').length;
			if (damage >= num) {
				return true;
			}
		},
	...
	North_bmh_lushou: {
		...
		content() {
			if (event.triggername == 'North_bmh_wuxian') {
				...
				if (get.North_bmh_chizhang(player).length) {
					trigger.num += 3;
				}
			} else {
				player.draw(3);
	...
	North_bls_qiangong: {
		...
		enable: 'phaseUse',
		getsuit(player) {
			if (player.storage.North_bls_qiangong_suit) {
				return player.storage.North_bls_qiangong_suit;
			}
			return [];
		},
		getsuit2(player) {
			if (player.storage.North_bls_qiangong_suit2) {
				return player.storage.North_bls_qiangong_suit2;
			}
			return [];
		},
		gettar1(player) {
			if (player.storage.North_bls_qiangong_tar1) {
				return player.storage.North_bls_qiangong_tar1;
			}
			return [];
		},
		gettar2(player) {
			if (player.storage.North_bls_qiangong_tar2) {
				return player.storage.North_bls_qiangong_tar2;
			}
			return [];
		},
		filter(event, player) {
		...
		filterCard(card, player) {
			...
			return !lib.skill.North_bls_qiangong.getsuit(player).includes(card.suit);
		},
		selectTarget: 2,
		filterTarget(card, player, target) {
		...
		intro: {
			content(event, player, storage, name, skill) {
				const suit = lib.skill.North_bls_qiangong.getsuit(player);
				const suit2 = lib.skill.North_bls_qiangong.getsuit2(player);
				const tar1 = lib.skill.North_bls_qiangong.gettar1(player);
				const tar2 = lib.skill.North_bls_qiangong.gettar2(player);
				let str = '';
				if (player.hasSkill('North_bls_qiangong_ban')) {
					str += '<span class=yellowtext>此技能本回合失效</span>';
				}
				str += '<br>计入次数的花色:';
		...
		content() {
			...
			('step 2');
			const suit = event.suit;
			const cards = targets[0].getCards('he', function (i) {
				return i.suit == suit;
			});
			...
			const suit1 = event.suit;
			if (cards1.length <= 2) {
				player.draw();
			} else {
				player.addTempSkill('North_bls_qiangong_suit');
				if (!player.storage.North_bls_qiangong_suit) {
					player.storage.North_bls_qiangong_suit = [];
				}
				player.storage.North_bls_qiangong_suit.add(suit1);
			}
			...
			if (!player.storage.North_bls_qiangong_suit2) {
				player.storage.North_bls_qiangong_suit2 = [];
			}
			if (!player.storage.North_bls_qiangong_tar1) {
				player.storage.North_bls_qiangong_tar1 = [];
			}
			if (!player.storage.North_bls_qiangong_tar2) {
				player.storage.North_bls_qiangong_tar2 = [];
			}
			player.storage.North_bls_qiangong_suit2.add(suit1);
	...
	North_bls_yuanya: {
		...
		filter(event, player, name) {
			if (name == 'phaseEnd') {
				return lib.skill.North_bls_qiangong.getsuit2(player).length == 4;
			}
			return true;
		},
		content() {
			'step 0';
			let str = event.triggername == 'phaseBegin' ? '回合开始时' : '回合结束时';
			player
				.chooseTarget()
				.set('ai', function (target) {
					let att = get.attitude(player, target);
			...
			if (result.targets) {
				...
				let list2 = ['选项一:出牌阶段结束后额外获得1个摸牌阶段', '选项二:弃牌阶段结束后额外获得1个出牌阶段', '选项三:废除判定区'];
				if (!player.storage['North_bls_yuanya_' + result.targets[0].playerid]) {
					player.storage['North_bls_yuanya_' + result.targets[0].playerid] = [];
				}
				event.list = [];
	...
	North_cjy_bashu: {
		...
		filter(event, player) {
			if (player.storage.North_cjy_lvzhi > 0 || player.countCards('e') > 0) {
				return true;
			}
			return false;
		},
		content() {
			'step 0';
			const numaa = player.storage.North_cjy_lvzhi;
			if (player.countCards('e') > 0) {
				player
					.discardPlayerCard(player, [1, player.countCards('e')], 'e')
					.set('prompt', '请选择要弃置的牌')
		...
		intro: {
			content(storage, player) {
				let str = '本回合以下标黄的阶段改为出牌阶段';
				const list = ['判定阶段', '摸牌阶段', '弃牌阶段'];
				if (player.storage.North_cjy_bashu_1) {
					if (player.storage.North_cjy_bashu_1 > 0) {
						list[0] = '<span class=yellowtext>' + list[0] + '</span>';
					}
					if (player.storage.North_cjy_bashu_1 > 1) {
						list[1] = '<span class=yellowtext>' + list[1] + '</span>';
					}
					if (player.storage.North_cjy_bashu_1 > 2) {
						list[2] = '<span class=yellowtext>' + list[2] + '</span>';
					}
				}
				str += list;
				return str;
			...
			1: {
				...
				filter(event, player, name) {
					if (name == 'phaseJudgeBefore') {
						return player.storage.North_cjy_bashu_1 > 0;
					}
					if (name == 'phaseDrawBefore') {
						return player.storage.North_cjy_bashu_1 > 1;
					}
					if (name == 'phaseDiscardBefore') {
						return player.storage.North_cjy_bashu_1 > 2;
					}
					return false;
				},
				forced: true,
	...
	North_cjy_duijing: {
		...
		content() {
			if (player.storage._YB_phaseNumber % 4 == 0) {
				let num = 5;
			}
			player.draw(num || 1);
		},
		prompt2(event, player) {
			let str = '你可以摸';
			let num = player.storage._YB_phaseNumber % 4 == 0 ? 5 : 1;
			str += num;
			str += '张牌';
			return str;
		},
		...
		intro: {
			...
			content(event, player) {
				let num = player.storage._YB_phaseNumber || 0;
			...
			diancai: {
				...
				content() {
					...
					if (trigger.player.countCards('e') > 0) {
						list1.push('装备区');
						list1.push(trigger.player.getCards('e'));
					}
					if (trigger.player.countCards('j') > 0) {
						list1.push('判定区');
						list1.push(trigger.player.getCards('j'));
					}
	...
	North_cjy_lvzhi: {
		...
		trigger: { global: 'phaseAfter' },
		filter(event, player) {
			const tar = _status.currentPhase;
			if (player == tar) {
				return false;
			}
			const numa = tar.getHistory('useCard', function (evt) {
				if (evt.targets && evt.targets.length && evt.isPhaseUsing()) {
					const targets = evt.targets.slice(0);
					return targets.length;
				}
			...
			// game.log(numa);
			let numb = player.getHistory('lose', function (evt) {
				const ll = [];
				ll.add(evt.hs);
				ll.add(evt.es);
			}).length;
	...
	North_hyy_guishi: {
		...
		intro: {
			...
			mark(dialog, content, player) {
				content = player.getExpansions('North_hyy_guishi');
		...
		subSkill: {
			qixing: {
				...
				forced: true,
				filter(event, player) {
					return player.getExpansions('North_hyy_guishi').length && player.countCards('h') > 0;
				},
				content() {
					'step 0';
					const cards = player.getExpansions('North_hyy_guishi');
					if (!cards.length || !player.countCards('h')) {
						event.finish();
						return;
					}
					const next = player.chooseToMove('闺识·七星:是否交换<闺识>和手牌？');
					next.set('list', [
			...
			taoluan: {
				...
				filter(event, player) {
					...
					for (const i of player.getExpansions('North_hyy_guishi')) {
						const type = get.type(i);
						if (type == 'trick' && evt({ name: i.name }, player, event)) {
							return true;
						}
					}
					return false;
	...
	North_hyy_lancai: {
		...
		preHidden: true,
		filter(event, player) {
			return player.getExpansions('North_hyy_guishi').length;
		},
		content() {
			'step 0';
			let num = player.getExpansions('North_hyy_guishi').length;
			const cards = get.cards(num);
			game.cardsGotoOrdering(cards);
			const next = player.chooseToMove();
			next.set('list', [['牌堆顶', cards], ['牌堆底']]);
		...
		subSkill: {
			guanxu: {
				...
				usable: 1,
				filter(event, player) {
					return player.getExpansions('North_hyy_guishi').length && game.hasPlayer((current) => lib.skill.North_hyy_lancai_guanxu.filterTarget(null, player, current));
				},
				filterTarget(card, player, target) {
				...
				content() {
					...
					const dialog = ['兰才·观虚:选择要操作的牌', '<div class="text center">' + get.translation(target) + '的手牌</div>', hs, '<div class="text center">(闺识)</div>', cards];
					player
						...
						.set('cards2', cards)
						.set('ai', function (button) {
							const card = button.link, cards1 = _status.event.cards1.slice(0);
					...
					('step 2');
					game.updateRoundNumber();
					const suits = [], map = {}, hs1 = target.getCards('h');
					if (hs1.length) {
						for (const i of hs1) {
							if (!lib.filter.canBeDiscarded(i, player, target, 'North_hyy_lancai_guanxu')) {
								continue;
							}
							const suit = i.suit;
							if (!map[suit]) {
	...
	North_hyy_bingxue: {
		...
		mod: {
			targetEnabled(card, player, target) {
				const cards = target.getExpansions('North_hyy_guishi');
				const suit = [];
				for (const i of cards) {
					if (!suit.includes(i.suit)) {
						suit.push(i.suit);
	...
	North_cxch_lingxi: {
		...
		filter(event, player) {
			...
			// 	return evt!=event&&evt.card.suit==suit&&lib.skill.North_cxch_lingxi.hasHand(evt);
			// }).length==0;
		},
		ai: {
			result: {
		...
		subSkill: {
			yuping: {
				...
				intro: {
					...
					mark(dialog, storage, player) {
						if (player.getExpansions('North_cxch_lingxi_yuping').length) {
							dialog.add(player.getExpansions('North_cxch_lingxi_yuping'));
						} else {
							dialog.add('暂无玉娉');
						}
					},
			...
			tingniao: {
				...
				intro: {
					...
					markcount: 'expansion',
					mark(dialog, storage, player) {
						if (player.getExpansions('North_cxch_lingxi_tingniao').length) {
							dialog.add(player.getExpansions('North_cxch_lingxi_tingniao'));
						} else {
							dialog.add('暂无婷袅');
						}
					},
	...
	North_cxch_gongsheng: {
		...
		filter(event, player) {
			if (event.name == 'addToExpansion') {
				if (event.gaintag.includes('North_cxch_lingxi_yuping')) {
					const cards2 = event.player.getExpansions('North_cxch_lingxi_yuping').filter((item) => {
						for (const k of event.cards) {
							return item != k;
						}
					});
					const cards = event.player.getExpansions('North_cxch_lingxi_yuping');
					const cards3 = event.player.getExpansions('North_cxch_lingxi_yuping').concat(event.cards);
					...
					const cards2 = event.player.getExpansions('North_cxch_lingxi_tingniao').filter((item) => {
						for (const k of event.cards) {
							return item != k;
						}
					});
					const cards = event.player.getExpansions('North_cxch_lingxi_tingniao');
					const cards3 = event.player.getExpansions('North_cxch_lingxi_tingniao').concat(event.cards);
		...
		subSkill: {
			give: {
				...
				filter(event, player) {
					const cards = player.getExpansions('North_cxch_lingxi_yuping');
	...
	North_cxch_lianyu: {
		...
		filter(event, player, name) {
			const cards = player.getExpansions('North_cxch_lingxi_tingniao');
			if (name == 'damageBegin4') {
			...
			const result = await player
				...
				.set('ai', function () {
					if ((event.triggername += 'useCardToPlayer')) {
						...
						const eff = get.damageEffect(trigger.source, trigger.player);
						return att > 0 && eff < 0;
					}
				})
				.forResult();
	...
	North_cxch_xixuan: {
		...
		filter(event, player) {
			let evt = lib.filter.filterCard;
			const cards = event.player.getExpansions('North_cxch_lingxi_yuping');
			const cards2 = event.player.getExpansions('North_cxch_lingxi_tingniao');
			if (event.filterCard) {
				evt = event.filterCard;
			}
			for (const i of lib.inpile) {
				const type = get.type(i);
				if ((type == 'trick' && evt({ name: i }, player, event) && cards.length) || (type == 'basic' && evt({ name: i }, player, event) && cards2.length)) {
					return true;
				}
			}
			return false;
		...
		hiddenCard(player, name) {
			const cards = player.getExpansions('North_cxch_lingxi_yuping');
			const cards2 = player.getExpansions('North_cxch_lingxi_tingniao');
			const type = get.type(name);
		...
		chooseButton: {
			dialog(event, player) {
				const list = [];
				player = _status.event.player || player;
				const cards = player.getExpansions('North_cxch_lingxi_yuping');
				const cards2 = player.getExpansions('North_cxch_lingxi_tingniao');
				for (let i = 0; i < lib.inpile.length; i++) {
					if (get.type(lib.inpile[i]) == 'trick' && cards.length) {
						list.push(['锦囊', '', lib.inpile[i]]);
					} else if (get.type(lib.inpile[i]) == 'basic' && cards2.length) {
						if (lib.inpile[i] == 'sha') {
							for (const kkk of lib.inpile_nature) {
								switch (kkk) {
									case 'fire':
	...
	North_shh_yuniao: {
		...
		intro: {
			content(storage, player, skill) {
				...
				if (player.storage.North_shh_yuniao == true) {
					str += '转换技.回合结束时,<span class=thundertext>阳:你可获得1名角色一张牌,然后对该角色造成1点火属性伤害</span>;阴:你可交给1名角色一张牌,然后对该角色造成1点伤害';
				} else {
					str += '转换技.回合结束时,阳:你可获得1名角色一张牌,然后对该角色造成1点火属性伤害;<span class=thundertext>阴:你可交给1名角色一张牌,然后对该角色造成1点伤害</span>';
				}
				return str;
			},
		},
		...
		content() {
			...
			player
				...
				.set('ai', function (target) {
					let att = get.attitude(_status.event.player, target);
					const eff = get.damageEffect(target, _status.event.player, nat);
					return att < 0 && eff >= 0;
				});
			('step 1');
			if (result.bool) {
				if (player.storage.North_shh_yuniao == true) {
					event.list = [player, result.targets[0], 'fire', result.targets[0], 'fire'];
				} else {
					event.list = [result.targets[0], player, 'thunder', result.targets[0], null];
				}
			...
			player.chooseCardTarget({
				...
				filterTarget(card, player, target) {
					...
					if (player.inRange(target)) {
						if (lib.filter.targetEnabled({ name: 'sha', nature: 'thunder' }, player, target)) {
							return true;
						}
					}
					return false;
	...
	North_shh_xianyin: {
		...
		content() {
			...
			const all = player.getAllHistory('sourceDamage');
			if (!all.length) {
				event.finish();
			} else {
				const dam = all[all.length - 1];
	...
	North_gz_shujian: {
		...
		selectCard() {
			const player = _status.event.player;
			let num = player.getStat('skill').North_gz_shujian ? player.getStat('skill').North_gz_shujian + 1 : 1;
			return [num, Infinity];
		},
		filterCard(card, player) {
			let num = 0;
		...
		check(card) {
			...
			let num = player.getStat('skill').North_gz_shujian;
	...
	North_zx_huashuang: {
		...
		subSkill: {
			max: {
				...
				content() {
					player.storage.North_zx_huashuang_max = true;
					player.gainMaxHp();
				},
				limited: true,
			},
	...
	North_zx_ningao: {
		...
		subSkill: {
			use: {
				...
				ai: {
					...
					skillTagFilter(player, tag) {
						...
						if (player.maxHp - player.hp < 3 && !player.countCards('hes', { suit: 'diamond' })) {
							return false;
						}
				...
				hiddenCard(player, name) {
					if (name == 'tao') {
						return player.countCards('hes', { suit: 'heart' }) > 0 && player.maxHp - player.hp >= 3;
					}
				},
			},
			bit: {
			...
			long: {
				...
				content() {
					'step 0';
					const next = player.chooseToUse(get.prompt('North_zx_ningao_long'), { name: 'sha' });
					next.aidelay = true;
					next.noButton = true;
					('step 1');
					if (result.bool) {
			...
			jue: {
				...
				filter(event, player) {
					...
					for (let i = 0; i < 8; i++) {
						...
						if (evt.name != 'North_zx_ningao_jue') {
							return true;
						}
					}
					return false;
	...
	North_lf_zhenzhi: {
		...
		content: async function (event, trigger, player) {
			if (!player.storage.North_lf_zhenzhi) {
				player.storage.North_lf_zhenzhi = [];
			}
			...
			if (event.triggername == 'phaseZhunbeiBegin') {
				...
				if (result.bool) {
					...
					let num = player.storage.North_lf_zhenzhi.length || 0;
	...
	North_PC_qianshui: {
		...
		filter(event, player, name) {
			if (name == 'roundStart') {
				...
				if (event.name == 'lose') {
					for (let i in event.gaintag_map) {
						if (event.gaintag_map[i].includes('North_PC_qianshui_tag')) {
							return true;
						}
					}
					return false;
				...
				return player.hasHistory('lose', function (evt) {
					...
					for (let i in evt.gaintag_map) {
						if (evt.gaintag_map[i].includes('North_PC_qianshui_tag')) {
							return true;
						}
					}
					return false;
		...
		content() {
			...
			if (trigger.name == 'lose') {
				for (let i in trigger.gaintag_map) {
					if (trigger.gaintag_map[i].includes('North_PC_qianshui_tag')) {
						num++;
					}
				}
			} else {
		...
		mod: {
			aiValue(player, card, num) {
				if (get.itemtype(card) != 'card' || !card.hasGaintag('North_PC_qianshui_tag')) {
					return;
				}
				if (get.distance(_status.currentPhase, player, 'absolute') == 1 && !player.isTurnedOver()) {
					return;
				}
				if (
		...
		intro: {
			...
			mark(dialog, content, player) {
				const cards = player.getCards('h', (card) => card.hasGaintag('North_PC_qianshui_tag'));
				if (cards.length) {
	...
	caoyi_miyi: {
		...
		content() {
			...
			event.list = ['回复一点体力', '受到一点你造成的伤害'];
			player
				.chooseControl('cancel2')
				.set('choiceList', event.list)
				.set('prompt', get.prompt('caoyi_miyi'))
				.set('ai', function () {
					if (
						game.countPlayer(function (current) {
			...
			('step 2');
			let str = '令这些角色各' + event.list[event.numb] + ',若如此做,结束阶段这些角色执行另一项';
			player
				.chooseTarget([1, Infinity], '请选择任意名角色')
				.set('prompt2', str)
				.set('ai', function (target) {
					let att = get.attitude(player, target);
					const att2 = get.damageEffect(target, player, player);
					if (event.numb == 0) {
		...
		subSkill: {
			add: {
				...
				filter(event, player) {
					return player.storage.caoyi_miyi && player.storage.caoyi_miyi.length && player.storage.caoyi_miyi_add;
				},
				content() {
					'step 0';
	...
	caoyi_yinjun: {
		...
		filter(event, player) {
			if (player.hasSkill('caoyi_yinjun_add')) {
				return false;
			}
			...
			if (!lib.filter.targetEnabled2({ name: 'sha' }, player, event.targets[0])) {
				return false;
			}
			return true;
		},
		...
		subSkill: {
			mark: {
				...
				filter(event, player) {
					let num = player.storage.caoyi_yinjun_mark;
					if (!num) {
						return false;
					}
	...
	// 			function(card){return card.name=='sha'}
	// 		]
	// 	},
	// 	init(player){
	...
	// 			players.add(tar.next);
	// 			players.add(tar.previous);
	// 			players.sortBySeat();
			...
			if (event.numb < event.targets.length) {
				...
				if (event.tar && event.tar.isIn()) {
					event.tar
						.chooseCard('he', 1)
						.set('prompt', '号令')
						.set('prompt2', '清选择一张手牌交给' + get.translation(player) + ',否则受到一点伤害');
				}
			} else {
				event.goto(3);
			}
		...
		subSkill: {
			add: {
				...
				filter(event, player, cards) {
					return game.hasPlayer(function (current) {
						const es = current.getCards('ej', function (card) {
					...
					if (!lib.skill.North_haoling.filter(null, player, cards)) {
						...
						const next = player.chooseTarget(2, function (card, player, target) {
							if (ui.selected.targets.length) {
								...
								const es = from.getCards('ej', function (card) {
						...
						next.set('ai', function (target) {
							...
							if (ui.selected.targets.length == 0) {
								if (target == player && player.hasSkill('decadexuanfeng')) {
									if (
										player.countCards('ej', function (card) {
											return (
									...
									if (
										game.hasPlayer(function (current) {
											if (current != target && get.attitude(player, current) > 0) {
												const es = target.getCards('ej', function (card) {
													return !_status.event.cards.includes(card);
												});
												for (let i = 0; i < es.length; i++) {
							...
							const es = ui.selected.targets[0].getCards('ej', function (card) {
								return !_status.event.cards.includes(card);
							});
							let i;
	...
	North_zhenglue: {
		...
		content() {
			...
			player
				.chooseTarget(1, true)
				.set('ai', function (target) {
					const playerx = _status.event.player || player;
					if (playerx) {
						if (playerx.isIn()) {
							return (target = playerx);
						}
					}
		...
		subSkill: {
			add: {
				...
				filter(event, player) {
					if (player.countMark('North_zhenglue_add') >= 3) {
						return true;
					}
					return false;
				},
	...
	North_mouhua: {
		...
		enable: ['chooseToUse', 'chooseToRespond'],
		filter(event, player) {
			if (player.countMark('North_mouhua_block') >= 3) {
				return false;
			}
			if (player.countCards('hes') < 1) {
				return false;
			...
			for (const i of lib.skill.North_mouhua.basic) {
				if (evt({ name: i }, player, event)) {
					return true;
				}
			}
			for (const i of lib.skill.North_mouhua.trick) {
				if (evt({ name: i }, player, event)) {
					return true;
				}
			}
			for (const i of lib.skill.North_mouhua.delay) {
				if (evt({ name: i }, player, event)) {
					return true;
				}
			}
		...
		trick: ['guohe', 'shunshou', 'wuzhong', 'wugu', 'wuxie', 'taoyuan', 'wanjian', 'nanman', 'juedou', 'jiedao', 'huogong', 'tiesuo', 'suijiyingbian', 'dongzhuxianji', 'shuiyanqijunx', 'zhujinqiyuan', 'chenghuodajie', 'tuixinzhifu', 'guaguliaodu'],
	...
	YB_nobody_guiyin: {
		...
		filter(event, player, name) {
			...
			if (players.length != 1) {
				return name == 'useCard' && ['basic', 'trick'].includes(get.type(event.card));
			} else if (players.length == 1 && name == 'gainEnd' && players[0] == event.player) {
				return true;
			} else {
				return false;
			}
		...
		content() {
			...
			('step 2');
			player
				.chooseTarget('癸隐:是否为' + get.translation(trigger.card) + '增加任意目标？', [1, Infinity], function (card, player, target) {
	...
	YB_nobody_linglv: {
		...
		content() {
			...
			('step 1');
			const players = [];
			players.add(player.next);
			players.add(player.previous);
			for (const i of players) {
		...
		subSkill: {
			mark: {
				...
				filter(e, p) {
					return p.storage.YB_nobody_linglv_mark;
				},
				content() {
					trigger.num += player.storage.YB_nobody_linglv_mark;
				},
				mark: true,
				intro: {
				...
				mod: {
					globalTo(from, to, distance) {
						const numa = 0;
						if (to.storage.YB_nobody_linglv_mark) {
							const numa = to.storage.YB_nobody_linglv_mark;
						}
	...
	YB_nobody_longying: {
		...
		content() {
			...
			('step 0');
			let num = target.getHistory('useCard').length;
			player
				.chooseToDiscard(num, 'he', '是否弃置' + num + '张牌,然后弃置' + get.translation(target) + '等量的牌？<br>若因此弃置了不少于两张牌,你对' + get.translation(target) + '造成一点伤害')
	...
	YB_nobody_fengguo: {
		...
		longying(a) {
			const his = a.getAllHistory('damage', (evt) => {
				//if(evt.parent)game.log('evt.parent:',evt.parent);
				//if(evt.parent.name)game.log('evt.parent.name:',evt.parent.name);
				...
				return evt.parent.name == 'YB_nobody_longying';
		...
		content() {
			let target = trigger.player;
			('step 0');
			const list = ['与' + get.translation(player) + '各摸两张牌', '本局游戏对因<龙缨>受到过伤害的角色造成的伤害＋1'];
			if (!target.hasSkill('YB_nobody_fengguo_mark')) {
				target.chooseControl('摸牌', '加伤').set('choiceList', list);
			} else {
		...
		subSkill: {
			mark: {
				...
				intro: {
					content(event, player) {
						const list = game.filterPlayer((a) => lib.skill.YB_nobody_fengguo.longying(a));
						if (list.length) {
							return '对' + get.translation(list) + '造成伤害时,此伤害+1';
						}
						return '无';
					},
				},
	...
	// 		if (event.name == 'chooseButton') return event.type == 'chooseToUse_button' && event.parent.result.skill == 'yuanxing';
	// 		return (!event.customArgs || !event.customArgs.yuanxing) && lib.inpile.some(name => event.filterCard({name}, player, event));
	// 	},
	// 	filterCard: () => false,
	...
	// 		},
	// 		filter ({link}) {
	// 			const {event, event: {player}} = _status, parent = event.parent;
	...
	// 	},
	// 	content: async function (event, trigger, player) {
	// 		const parent = trigger.parent, targets = game.filterPlayer(target => target != player);
