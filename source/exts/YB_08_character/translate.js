...
const translate = {
	...
	ybmjz_jianxiong: '奸雄',
	ybmjz_jianxiong_info: `当你受到伤害后,你可以获得对你造成伤害的牌并摸X张牌(X为本次伤害值且至少为1).当你以此法获得牌时,你展示并记录这些牌点数(重复点数不记录),然后若已记录点数不少于9个,你依次执行:①增加1点体力上限;②获得你武将牌上未生效的主公技;③修改〖奸雄〗.`,
	ybmjz_jianxiong_gai: '奸雄·改',
	ybmjz_jianxiong_gai_ab: '奸雄',
	...
	ybmjz_rende: '仁德',
	ybmjz_rende_info: '出牌阶段限一次,你可以将任意张手牌置入仁库,同时指定至多等量其他角色,你令他们各摸两张牌;每回合限一次,你可以将一张仁库牌当一张基本牌使用;当仁库牌溢出时,你可将溢出牌分配给其他角色,然后获得牌的角色可以<拥戴>你(已拥戴改为可以令你回复一点体力);每局限一次,准备阶段,若存活的<拥戴>你的角色大于存活角色数三分之一,你加一点体力上限,回复一点体力,获得你未拥有的主公技',
	// 'ybmjz_rende_info':'出牌阶段限一次,你可以将任意张手牌或仁库中的牌交给一名其他角色,然后声明一个基本或非延时锦囊牌,其可以展示一张手牌令此牌视为之.若如此做,你可以回复1点体力或视为使用你声明的牌',
	ybmjz_xiemin: '携民',
	ybmjz_xiemin_info: `主公技,每回合每名角色限一次,其他角色使用有实体牌的转化牌或【杀】后,其可以将位于处理区的此牌置入仁库,然后若其不为蜀势力,你可令其改为蜀势力,否则你可以发动${get.poptip('ybmjz_rende')}的第一句.`,
	ybmjz_guanyu: '名将关羽',
	ybmjz_guanyu_prefix: '名将',
	ybmjz_yijue: '义绝',
	ybmjz_yijue_info: '锁定技,当你使用【杀】指定唯一目标时,你暗中选择一项:1、令此【杀】效果改为回复,若此【杀】被抵消,你摸两张牌,然后你对其使用牌无次数限制、其不能使用或打出手牌且非锁定技失效直到回合结束;2、此【杀】伤害+1',
	ybmjz_yijuex: '义绝',
	ybmjz_yijuex_info: '锁定技,当你使用【杀】指定唯一目标时,你暗中选择一项:1、令此【杀】效果改为回复;2、此【杀】伤害+1.<br>然后其选择一项:1、交给你一张手牌,令此【杀】无效;2、令其不可响应此【杀】.若其与你选择的序号相同,此【杀】不计入次数且本回合本技能其的选择改为你代替其选择',
	ybmjz_zhangfei: '名将张飞',
	...
	ybmjz_limu: '立牧',
	ybmjz_limu_info: '出牌阶段,你可以将一张:♠️️️牌当【闪电】,♥️️️牌当【火山】,♣️️️牌当【洪水】,♦️️️牌当【随波逐流】 对自己使用.只要你的判定区内有牌,你对攻击范围内的其他角色使用牌便没有次数和距离限制',
	ybmjz_sunluyu: '名将孙鲁育',
