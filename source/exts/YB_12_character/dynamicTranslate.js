...
const dynamicTranslate = {
	...
	sgskjdbzjms_fenglei(player) {
		...
		if (storage) {
			...
			if (player.hasSkill('sgskjdbzjms_fenglei_wind')) {
				return '<span class=yellowtext>' + str + '</span>' + str2 + '<span class=yellowtext>' + strwind + '</span>;' + strthunder + '';
			}
	...
	qmsgswkjsgj_shhlianhua(player) {
		return ['你成为其他角色【杀】的目标后,你摸一张牌,然后进行一次判定,若结果为♠️️️,则取消之', '你成为其他角色【杀】的目标后,你摸一张牌,除非该角色弃置一张牌,否则取消之,然后进行一次判定,若结果为♠️️️,则取消之', '你成为其他角色【杀】的目标后,你摸一张牌,除非该角色弃置一张牌,否则取消之,然后进行一次判定,若结果为黑色,则取消之'][player.countMark('qmsgswkjsgj_shhlianhua')];
	},
	...
	qmsgswkjsgj_shenci_shhlianhua(player) {
		return ['你成为其他角色【杀】的目标后,你摸一张牌,除非该角色弃置一张牌,否则取消之,然后进行一次判定,若结果为♠️️️,则取消之', '你成为其他角色【杀】的目标后,你摸一张牌,除非该角色弃置一张牌,否则取消之,然后进行一次判定,若结果为黑色,则取消之', '你成为其他角色牌的目标后,你可以摸一张牌,除非该角色弃置一张牌,否则取消之,然后进行一次判定,若结果不为♥️️️,则取消之'][player.countMark('qmsgswkjsgj_shenci_shhlianhua')];
	},
	qmsgswkjsgj_potkuanggu(player) {
