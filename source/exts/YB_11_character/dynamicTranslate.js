...
const dynamicTranslate = {
	...
	yhky_dcjiuchi(player) {
		if (player.storage.yhky_dchuayan_dcjuewu) {
			return get.poptip('rule_chihengji') + '。你可以将一张♠牌当作【酒】使用。你使用【酒】无次数限制。';
		}
	...
	yhky_dcchanjuan(player) {
		if (player.storage.yhky_dchuayan_dcjuewu) {
			return get.poptip('rule_chihengji') + '。当其他角色于你的回合内使用牌时，你移去6点魅力值。然后依次执行①摸两张牌；②令此牌基础伤害或恢复值+1；③使其本回合获得' + get.poptip('wushuang') + '。';
