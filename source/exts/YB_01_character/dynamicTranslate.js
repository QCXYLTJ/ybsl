...
const dynamicTranslate = {
	...
	yb033_qijue(player) {
		...
		if (storage1 && storage2 && storage3) {
			...
			return '锁定技,出牌阶段开始时,你选择一项:失去1点体力,受到1点伤害,弃置一张牌.锁定技,当你失去体力后,①你下次失去体力后,②回复此数值*2点体力,然后③弃置一张手牌;当你受到伤害后,①你下次受到伤害后,②摸此数值*3张牌,然后③失去1点体力;当你弃置牌后,①你下次弃置牌后,②对所有其他角色各造成1点伤害,然后③令一个数字之后的效果向前错位.';
		}
	},
	yb037_kexie(player) {
		//咳血
	...
	yb069_wenhuan(player) {
		//温幻
		if (player.storage.yb069_wenhuan == true) {
			return '转换技,阳,当有角色受到伤害后,你可以令其武将牌复位并令其摸一张牌.<span class="bluetext">阴,当有角色回复体力时,你可令其翻面,并令此次回复效果+1.</span>';
		}
		return '转换技,<span class="bluetext">阳,当有角色受到伤害后,你可以令其武将牌复位并令其摸一张牌.</span>阴,当有角色回复体力时,你可令其翻面,并令此次回复效果+1';
	},
	//071
	ybsl_cuixing_spade(player) {
		//淬星*4
		...
		for (let i = 0; i < player.storage.ybsl_cuixing_spade.length; i++) {
			...
			if (player.storage.ybsl_cuixing_ban_spade.includes(player.storage.ybsl_cuixing_spade[i])) {
				str += '<span style="text-decoration: line-through;">【' + get.translation(player.storage.ybsl_cuixing_spade[i]) + '】</span>';
			} else {
				str += '【' + get.translation(player.storage.ybsl_cuixing_spade[i]) + '】';
			}
		}
		str += '使用或打出,属性为<span class=YB_snowtext>冰</span>';
		return str;
	},
	ybsl_cuixing_heart(player) {
		...
		for (let i = 0; i < player.storage.ybsl_cuixing_heart.length; i++) {
			...
			if (player.storage.ybsl_cuixing_ban_heart.includes(player.storage.ybsl_cuixing_heart[i])) {
				str += '<span style="text-decoration: line-through;">【' + get.translation(player.storage.ybsl_cuixing_heart[i]) + '】</span>';
			} else {
				str += '【' + get.translation(player.storage.ybsl_cuixing_heart[i]) + '】';
			}
		}
		str += '使用或打出,属性为<span class=yellowtext>血</span>';
		return str;
	},
	ybsl_cuixing_club(player) {
		...
		for (let i = 0; i < player.storage.ybsl_cuixing_club.length; i++) {
			...
			if (player.storage.ybsl_cuixing_ban_club.includes(player.storage.ybsl_cuixing_club[i])) {
				str += '<span style="text-decoration: line-through;">【' + get.translation(player.storage.ybsl_cuixing_club[i]) + '】</span>';
			} else {
				str += '【' + get.translation(player.storage.ybsl_cuixing_club[i]) + '】';
			}
		}
		str += '使用或打出,属性为<span class=YB_darktext>雷</span>';
		return str;
	},
	ybsl_cuixing_diamond(player) {
		...
		for (let i = 0; i < player.storage.ybsl_cuixing_diamond.length; i++) {
			...
			if (player.storage.ybsl_cuixing_ban_diamond.includes(player.storage.ybsl_cuixing_diamond[i])) {
				str += '<span style="text-decoration: line-through;">【' + get.translation(player.storage.ybsl_cuixing_diamond[i]) + '】</span>';
			} else {
				str += '【' + get.translation(player.storage.ybsl_cuixing_diamond[i]) + '】';
			}
		}
