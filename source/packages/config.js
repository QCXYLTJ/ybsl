...
export const config = {
	...
	ybsl_wujinshilian: {
		name: '无尽试炼(需重启)',
		intro: '经实验发现,夜白国战和无尽试炼不可兼得,开了无尽试炼玩不了夜白国战,不知为何.<br><span class=yellowtext>夜白国战</span>:<br>将夜白包武将加入国战.<br>平衡有待调整.<br><span class=yellowtext>夜白旅程</span>:<br>已挑战过的敌人会作为可选队友出现在列表中;<br>加入选项:弃置五张牌,然后凭空印制花中四君子各一张.<br><span class=yellowtext>夜白塔防</span>:<br>可以自由点将;<br>其它有待魔改',
		init: 'lc',
		item: {
	...
	ybsl_loglog: {
		name: 'log发生器(已禁用)',
		intro: '开启后,场上角色每次执行操作都会log一遍所有参数.(参数或时机有遗漏欢迎补充)<br>由于会导致卡顿,请在适时使用.<br>如果由于私自开启导致出现无法关闭,重启的bug,请游戏外前往主代码区把条件检测改成别的',
		init: '关闭',
		item: {
...
export const help = {
	夜白神略: '总有游客玩我的扩展不开将包,然后找不到别的武将呢,说没意思.现在我希望你能耐心读指引.<br>首先打开菜单,去武将列表下翻找到夜白将包和六艺篇,打开,然后去卡牌列表下翻找到夜白牌堆,打开(BOSS搬运看心情开),然后重启就OK了',
	六艺篇: '<li>关于六艺系列<br>六艺区是夜白神略包创立的机制.六艺区相当于一个私人的木牛流马,可以如手牌般使用或打出.<br>出牌阶段限一次,你可以将一张手牌置入六艺区,然后若六艺区牌数大于等于3,则获得精艺(精艺:每当你失去一张六艺区的牌时,你摸一张牌.)六艺区至多为6.<br>与此同时,作者同时推出了六艺技,其特征是可以通过对六艺区牌的应用来触发效果.需要注意的是,某些技能虽然和六艺区有关,但作者并没有给它定义为六艺技,因为它只是额外的把牌放入六艺区而并没有利用',
	// "结阵技":"<li>关于结阵技"
