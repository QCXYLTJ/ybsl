export { YB_yebailvcheng };
const YB_yebailvcheng = function () {
	...
	lib.skill._YBLC_level = {
		...
		content: function () {
			'step 0';
			if (!player.storage._YBLC_maxhp) {
				player.storage._YBLC_maxhp = 0;
			}
			if (!player.storage._YBLC_hand) {
				player.storage._YBLC_hand = 0;
			}
			if (!player.storage._YBLC_draw) {
				player.storage._YBLC_draw = 0;
			}
			if (!player.storage._YBLC_damage) {
				player.storage._YBLC_damage = 0;
			}
			if (!player.storage._YBLC_damage2) {
				player.storage._YBLC_damage2 = 0;
			}
			('step 1');
			...
			3: {
				...
				mod: {
					...
					cardUsable: function (card, player, num) {
						if (card.name == 'sha') {
							return num + (player.storage._YBLC_hand || 0);
						}
					},
				},
	...
	if (lib.brawl) {
		lib.brawl.yebailvcheng = {
			...
			intro: ['夜白的魔改，尔将进行无尽而漫长的单挑试炼', lib.config.yebailvcheng_level ? '你的最高纪录是连续通过' + lib.config.yebailvcheng_level + '关，是否能够突破这一记录呢？' : '你能否过五关斩六将，击败古城战神蔡阳呢？'],
			init: function () {
				if (!_status.yebailvcheng) {
					_status.yebailvcheng = {
						completeNumber: 0,
				...
				*/
					...
					*/
							...
							game.zhu
								.chooseControl()
								.set('choiceList', list)
								.set('prompt', '请选择一项奖励（当前已通过' + _status.yebailvcheng.completeNumber + '关）');
							...
							// else if(_status.yebailvcheng.completeNumber==10) {event._result={links:['ybsl_022salt']};}
								...
								game.zhu.chooseButton(['选择下一关出战的对手', [_status.characterlist.randomGets(lib.config.extension_夜白神略_ybsl_wujianghouxuan), 'character']], true);
							}
							//-------------------------------------------------此处数值为第二关及以后候选敌人的数目↑↑--------------//
							/*
					...
					*/
							...
							_status.yebailvcheng.completeYbsl.push([
								'在下一关中召唤' + get.translation(name) + '一同战斗',
								function () {
					...
					game.chooseCharacter = function () {
						...
						next.setContent(function () {
							...
							_status.yebailvcheng.completeYbsl.push([
								...
								function () {
									_status.yebailvcheng.addFellow(result.links[0]);
								},
							]);
							var pack = {
								character: {
